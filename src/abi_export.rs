...
//! `transcode_health` / `memory_guard` + the `endpoint_resource!` CRUD) through
//! the toolkit's re-exported dispatch surface.
//!
...
extern "C" fn target_software() -> RString {
    RString::from("jellyfin")
}

extern "C" fn target_compat() -> RString {
    RString::from("10.8-10.10")
}
...
/// only its own `jellyfin.*` namespace across the ABI; the host already owns the
/// domain tools and would otherwise reject the manifest as colliding built-ins.
const TOOL_PREFIX: &str = "jellyfin.";

/// The plugin's own tool surface: `tool_manifest_json()` filtered to the
/// `jellyfin.*` namespace. Shared by `manifest()` (serialized back out) and
